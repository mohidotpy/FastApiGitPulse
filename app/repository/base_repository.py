...
class BaseRepository:
    ...
    def read_by_options(self, schema, eager=False):
        with self.session_factory() as session:
            ...
            ordering = schema_as_dict.get("ordering", setting.ORDERING)
            order_query = (
                getattr(self.model, ordering[1:]).desc()
                if ordering.startswith("-")
                else getattr(self.model, ordering).asc()
            )
            page = schema_as_dict.get("page", setting.PAGE)
            page_size = schema_as_dict.get("page_size", setting.PAGE_SIZE)
    ...
    def create(self, schema):
        with self.session_factory() as session:
            query = self.model(**schema.to_dict())
            try:
                session.add(query)
                session.commit()
