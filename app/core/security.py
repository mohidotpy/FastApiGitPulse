...
def create_access_token(subject: dict, expires_delta: timedelta = None) -> (str, str):
    ...
    encoded_jwt = jwt.encode(payload, setting.SECRET_KEY, algorithm=ALGORITHM)
    expiration_datetime = expire.strftime(setting.DATETIME_FORMAT)
...
def decode_jwt(token: str) -> dict:
    try:
        decoded_token = jwt.decode(token, setting.SECRET_KEY, algorithms=ALGORITHM)
